namespace ET.Server
{
	...
	public class C2G_EnterMapHandler : AMRpcHandler<C2G_EnterMap, G2C_EnterMap>
	{
		protected override async ETTask Run(Session session, C2G_EnterMap request, G2C_EnterMap response)
		{
			...
			gateMapComponent.Scene = await SceneFactory.CreateServerScene(gateMapComponent, player.AccountId, IdGenerater.Instance.GenerateInstanceId(), gateMapComponent.DomainZone(), "GateMap", SceneType.Map);
			...
			response.MyId = player.AccountId;
